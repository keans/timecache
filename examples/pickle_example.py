...
from timecache import Cache, PickleBackend, ONE_MINUTE, FIVE_MINUTES, \
    FIFTEEN_MINUTES


def main():
    # create cache with pickle backend
    cache = Cache(backend=PickleBackend())

    if not cache.is_existing:
        # create a new cache, add three key, value pairs with different
        # caching durations and save them persistently as pickle file
